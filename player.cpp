...
void Player::move(vector<vector<int> >& map)
{
		...
		for (int j = start; j < end; j++)
		{
			...
			if (collision(&destrect, &srcRect))
			{
				...
				if (map[i][j] == -4)
				{
					...
					{
						hud.RenderText("Find A Key", NULL, NULL, renderer, true);
				...
				else if (physics.box.x <= destrect.x + destrect.w && physics.box.y + physics.box.h >= destrect.y + 10)
				{
					...
					// This code is written by Mridul works for tiles
					//std::cout << "collision type 4" << endl;
					//physics.velocity.x = 0;
	...
	//physics.box.x += physics.velocity.x;
	//physics.box.y += physics.velocity.y;
	if (isFiring)
	{
		if (direction == 'r')
			frame = 19;
