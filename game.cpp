...
game::game(SDL_Window* windowfrommenu, SDL_Renderer* rendererfrommenu, Module *menu)
{
	...
	key = new Collectables(keyImg, 1, 30);
	door = new Collectables(doorImg, 1, 50);
...
game::~game()
{
	...
	SDL_DestroyTexture(keyImg);
	SDL_DestroyTexture(doorImg);
		...
		for (int j = start; j < end; j++)
			if (map[i][j] != 0)
			{
				...
				if (map[i][j] == -3)
				{
					...
					key->Show(j * TILE_SIZE - Mother::coord.x, i * TILE_SIZE, renderer);
					continue;
				}

				if (map[i][j] == -4)
				{
					door->Show(j * TILE_SIZE - Mother::coord.x, i * TILE_SIZE, renderer);
					continue;
				}
					...
					if (SDL_GetTicks() - fireTime > 500) {
						if (Player1->getDirection() == 'r') {
							...
							bullets.push_back(new bullet(LoadImage("bullet.png"),
								Player1->GetRect()->x + Player1->GetRect()->w, Player1->GetRect()->y, 5, 0));
						}
						else {
							Player1->SetFiring(true);
							bullets.push_back(new bullet(LoadImage("bullet.png"), Player1->GetRect()->x, Player1->GetRect()->y, -5, 0));
						}
					}
					break;
				}
...
int game::ShowOptions()
{
	...
	saveOptionButtons[0] = LoadText(font, slot[0].c_str(), {255, 0, 0});
	saveOptionButtons[1] = LoadText(font, slot[1].c_str(), {255, 0, 0});
	saveOptionButtons[2] = LoadText(font, slot[2].c_str(), {255, 0, 0});
	in.close();
